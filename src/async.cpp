...
void ASync::multi_fetch_messages( void )
{
  ...
  while ( ( message = curl_multi_info_read( m_multi_handle, &pending ) ) )
  {
    ...
    case CURLMSG_DONE:
      ...
      //
      // Must be the last line as the wrapper can be deleted here, easy_handle may be invalid
      notify_wrapper( message->easy_handle, outcome_code( message ) );
      break;
    default:
      break;
    }
...
int ASync::multi_cb_timer( [[maybe_unused]] CURLM * p_multi, long p_timeout_ms, void * p_clientp )
{
  ...
  bool ok    = true;
  //
  ok = ok && self1 != nullptr; // always true
  //
  if ( p_timeout_ms < 0 ) // delete the timer
  {
    ok = ok && uv_timer_stop( &self1->m_uv_timer ) == 0;
  }
  else
  {
    // add or replace a timer
    ok = ok && uv_timer_start(
        &self1->m_uv_timer,
        []( uv_timer_t * handle )
        {
          ...
          int running_handles;
          curl_multi_socket_action( self2->m_multi_handle, CURL_SOCKET_TIMEOUT, 0, &running_handles );
          //
          self2->multi_fetch_messages(); // must be the last line as the wrapper can be deleted here
        },
        static_cast<uint64_t>( p_timeout_ms ), 0 ) == 0;
  }
  //
    ...
    void *                  p_socket_data )
{
  ...
  case CURL_POLL_INOUT:
    ...
    ok = ok && uv_poll_start( &context->poll, events, &uv_cb ) == 0;
    break;
...
void ASync::uv_cb( uv_poll_t * p_handle, [[maybe_unused]] int p_status, int p_events )
{
  ...
  //
  int running_handles;
  curl_multi_socket_action( context->async->m_multi_handle, context->curl, flags, &running_handles );
  //
...
// The negotiation headers are separated from the final headers
// by a blank line.
// Chunked should be ignored if received.
void ASync::notify_wrapper( CURL * p_curl, long p_result_code ) const
{
  ...
  if ( curl_easy_getinfo( p_curl, CURLINFO_PRIVATE, &wrapper ) == CURLE_OK )
    if ( wrapper != nullptr )
      wrapper->cb_back( p_result_code ); // must be the last line as the wrapper can be deleted here
}
