...
HTTP & HTTP::POST( const std::string & p_url, const std::string & p_content_type, const std::string & p_body )
{
...
HTTP & HTTP::PUT( const std::string & p_url, const std::string & p_content_type, const std::string & p_body )
{
...
HTTP & HTTP::PATCH( const std::string & p_url, const std::string & p_content_type, const std::string & p_body )
{
...
HTTP & HTTP::add_query_parameter( const std::string & p_key, const std::string & p_value )
{
  m_request_query_parameters.insert_or_assign( p_key, p_value );
  //
  return *this;
}
...
HTTP & HTTP::add_body_parameter ( const std::string & p_key, const std::string & p_value )
{
  m_request_body_parameters.insert_or_assign( p_key, p_value );
  //
  return *this;
}
...
bool HTTP::prepare_protocol( void )
{
  ...
  bool ok        = true;
  //
  ok = ok && easy_setopt( m_curl, CURLOPT_URL       , final_url.c_str()   ); // doesn't hae to be persistent
  ok = ok && easy_setopt( m_curl, CURLOPT_WRITEDATA , &m_response_body    );
  ok = ok && easy_setopt( m_curl, CURLOPT_HEADERDATA, &m_response_headers );
  //
  ok = ok && fill_method();
  ok = ok && fill_body();    // must be called before fill_headers()
...
bool HTTP::fill_method( void )
{
  ...
  //
  return easy_setopt( m_curl, CURLOPT_CUSTOMREQUEST, method );
}
...
bool HTTP::fill_headers( void )
{
  ...
  //
  ok = ok && easy_setopt( m_curl, CURLOPT_HTTPHEADER, m_curl_headers );
  //
  if ( ! ok )
  {
    curl_slist_free_all( m_curl_headers );
...
bool HTTP::fill_body_mime( void )
{
  ...
  //
  ok = ok && easy_setopt( m_curl, CURLOPT_MIMEPOST, m_curl_mime );
  //
  if ( ! ok )
  {
    easy_setopt( m_curl, CURLOPT_MIMEPOST, nullptr );
    //
    curl_mime_free( m_curl_mime );
    m_curl_mime     = nullptr;
